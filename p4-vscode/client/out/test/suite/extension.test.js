...
suite("P4 Language Server Integration", () => {
    ...
    test("Hover on struct definition returns summary", async () => {
        ...
        const contents = hovers[0].contents[0];
        const text = contents.value.toLowerCase();
        assert.ok(text.includes("struct") || text.includes("ethernet_t"), `Hover should mention struct or ethernet_t, got: ${text}`);
    });
    ...
    test("Completion at empty line returns keywords and locals", async () => {
        ...
        assert.ok(labels.some((l) => l === "bit" || l === "if" || l === "local_var"), `Should contain keywords or locals, got: ${labels.slice(0, 10).join(", ")}`);
    });
    test("Dot-triggered completion on struct field", async () => {
        ...
        const labels = completions.items.map((i) => i.label);
        assert.ok(labels.some((l) => l === "dstAddr" || l === "srcAddr" || l === "etherType"), `Should contain ethernet_t fields, got: ${labels.slice(0, 10).join(", ")}`);
    });
    // ======================== Go to Definition ========================
    test("Goto Definition on struct name", async () => {
        const doc = await openDoc(vscode.Uri.file(testP4));
        // Warm-up hover to ensure indexing
        await vscode.commands.executeCommand("vscode.executeHoverProvider", doc.uri, new vscode.Position(0, 7));
    ...
    test.skip("Rename local variable", async () => {
        ...
        const workspaceEdit = await vscode.commands.executeCommand("vscode.executeDocumentRenameProvider", doc.uri, pos, "new_local_var");
    ...
    test.skip("Document symbols returns struct, control, parser, action, table", async () => {
        const doc = await openDoc(vscode.Uri.file(testP4));
        const result = await vscode.commands.executeCommand("vscode.executeDocumentSymbolProvider", doc.uri);
        ...
        assert.ok(names.some((n) => n === "ethernet_t"), `Should contain ethernet_t, got: ${names.join(", ")}`);
        assert.ok(names.some((n) => n === "MyCtl"), `Should contain MyCtl, got: ${names.join(", ")}`);
        assert.ok(names.some((n) => n === "MyParser"), `Should contain MyParser, got: ${names.join(", ")}`);
    });
    // ======================== Signature Help ========================
    ...
    test.skip("Signature help on extern method call", async () => {
        ...
        const sigHelp = await vscode.commands.executeCommand("vscode.executeSignatureHelpProvider", doc.uri, pos, "(");
    ...
    test("Diagnostics detect issues", async () => {
        ...
        assert.ok(diagnostics.length > 0, `Should have diagnostics, got: ${JSON.stringify(diagnostics.map((d) => d.message))}`);
        ...
        const hasIssues = diagnostics.some((d) => d.message.toLowerCase().includes("undefined") ||
